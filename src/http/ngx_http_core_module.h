...
// 每个listen指令对应的结构体
typedef struct {
    in_addr_t  addr;// 监听的ip地址
    in_port_t  port; // 监听的端口
    int        family; // 地址簇
    ngx_str_t  file_name; // 监听unix域的文件
    int        line; // 在配置地址中的行数

    unsigned   default_server:1; // 是否是默认server，配置中用default_server指令说明
} ngx_http_listen_t;
...
// 
typedef struct {
    // 保存ngx_http_core_srv_conf_t结构体的数组
