...
void ngx_master_process_cycle(ngx_cycle_t *cycle, ngx_master_ctx_t *ctx)
{
    ...
    // 统计main函数接收参数中，全部字符串的大小，./nginx aa bb cc全部字符串
    for (i = 0; i < ctx->argc; i++) {
        size += ngx_strlen(ctx->argv[i]) + 1;
    }
    // 分配新的内存存储main函数参数
    title = ngx_palloc(cycle->pool, size);
    // 把master_process的n个字符复制到新的内存首地址，不包括'\0'，所以减一
    ...
    for ( ;; ) {
        if (delay) {
            ...
            // };  
            // 设置定时器，原理是延时it_value后触发alarm信号，每个it_interval后触发alarm，为0则不会再触发，it_value为0不会立即触发alarm信号
                                       ...
                                       ngx_int_t type)
{
    ...
    while (n--) {
        ...
        ch.fd = ngx_processes[ngx_process_slot].channel[0]; // 读通道
