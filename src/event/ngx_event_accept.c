...
void ngx_event_accept(ngx_event_t *ev)
{
    ...
    // 对应一个connection结构体,在ngx_event_process_init赋值的
    ls = ev->data;
    ...
    do {
        ...
        // 从accept队列摘取节点，connection的fd即listening的fd，sa存客户端的ip和端口
        s = accept(ls->fd, sa, &len);
        if (s == -1) {
            err = ngx_socket_errno;// 出错原因
    ...
    } while (ev->available);// 尽可以能多地accept
}


ngx_int_t ngx_trylock_accept_mutex(ngx_cycle_t *cycle)
{   // 同步变量等于0说明可以加锁，否则说明已经被其他进程加锁了，ngx_atomic_cmp_set是原子加锁操作
    if (*ngx_accept_mutex == 0
        && ngx_atomic_cmp_set(ngx_accept_mutex, 0, ngx_pid))
    {
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                       "accept mutex locked");
        // 拿到锁，并且ngx_accept_mutex_held为0，把本进程所有监听事件加到epoll中，如果为1说明之前已经拿到锁了
    ...
    // ngx_accept_mutex_held表示拿到锁，如果加锁失败，本进程该变量还是1说明有问题
    if (ngx_accept_mutex_held) {
        // 删除epoll中所有的监听事件
